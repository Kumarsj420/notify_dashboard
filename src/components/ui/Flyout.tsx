...
import { ChevronDownIcon, PhoneIcon, PlayCircleIcon, CursorArrowRippleIcon, StarIcon, UserIcon as UIcon } from '@heroicons/react/20/solid'
import { UserCircleIcon, BuildingOffice2Icon, UserIcon, AdjustmentsVerticalIcon, QuestionMarkCircleIcon } from '@heroicons/react/24/outline';
    ...
    return (
        <Popover className="relative">
            <PopoverButton className="inline-flex items-center gap-x-1 text-sm/6 font-semibold text-gray-900 outline-none ">
                ...
                <div className="size-8 rounded-full bg-linear-to-b from-sc-100 to-sc-200 flex items-center justify-center ring-1 ring-sc-300 outline-none ring-offset-2">
                    <UIcon aria-hidden='true' className='size-6 text-sc-600/90'/>
                </div>
                {user && (
                    <span className="hidden lg:flex lg:items-center">
                        <span aria-hidden="true" className="ml-4">
                            <span className='text-sm/6 font-semibold text-sc-900'>{user.name}</span>
            ...
            >
                <div className="w-screen max-w-[350px] flex-auto overflow-hidden rounded-3xl bg-white text-sm/6 shadow-lg shadow-gray-300/70 outline-1 outline-gray-900/5">
                    <div className="p-4">
                        {solutions.map((item) => (
                            <div key={item.name} className="group relative flex gap-x-6 rounded-lg py-3.5 px-4 hover:bg-gray-100">
                                <div className="mt-1 flex size-11 flex-none items-center justify-center rounded-lg bg-gray-100 group-hover:bg-white">
                                    <item.icon aria-hidden="true" className="size-6 text-gray-600/90 group-hover:text-p-600" />
                                </div>
                                <div>
                    ...
                    <div className="grid grid-cols-2 divide-x divide-gray-900/8 bg-gray-100">
                        <Link
                            href='/subscription'
                            className="flex items-center justify-center gap-x-2.5 p-3 font-semibold text-emerald-900 hover:bg-gray-50"
                        >
                            <StarIcon aria-hidden="true" className="size-5 flex-none text-emerald-400" />
                            Upgrade
                        </Link>
                        <button
                            onClick={logout}
                        ...
                        >
                            <CursorArrowRippleIcon aria-hidden="true" className="size-5 flex-none text-gray-400" />
                            Logout
                        </button>
                    </div>
                </div>
